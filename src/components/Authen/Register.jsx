...
export const Register = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (userInfo.username.length < 6) {
      return toast.error("username tối thiểu 6 kí tự", toastConfig);
    }
    ...
    if (userInfo.password < 6) {
      return toast.error("password tối thiểu 6 kí tự", toastConfig);
    }

    if (userInfo.password != userInfo.confirmPassword) {
      return toast.error("Mật khẩu xác nhận không trùng nhau", toastConfig);
    }

    if (document.getElementById("rule").checked === false) {
      return toast.error("Vui lòng xác nhận điều khoản", toastConfig);
    }

    const { confirmPassword, ...reqUserInfo } = userInfo;
    const result = await userApi.register(dispatch, {
      ...reqUserInfo,
      roleId: "644ea9444e00924f5be846d1",
    });
    console.log(result);
    if (result) {
          ...
          <div className="relative w-full sm:w-[500px] max-w-md px-4 h-full md:h-auto">
            <div className="bg-white rounded-lg shadow relative dark:bg-gray-700">
              ...
              <form className="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8" action="#">
                <h3 className="text-xl text-center font-bold text-gray-900 dark:text-white">Đăng kí</h3>
                <div>
                ...
                <div className="flex justify-between">
                  <div className="flex items-start">
                    ...
                    <div className="text-sm ml-3">
                      <label htmlFor="rule" className="font-medium text-gray-900 dark:text-gray-300">

...
export const Navbar = () => {
  ...
  return (
    <div
      className={`navbar bg-primary-sendo fixed w-full z-20 top-0 left-0 ${
    ...
    >
      <TopNav></TopNav>
      <div
        className={`flex mt-5 sm:mt-0 sm:pt-1 xl:max-w-[1308px] lg:max-w-[900px] md:max-w-[750px] mx-auto  ${
          isCategoryPage && window.innerWidth < 640 ? "hidden" : ""
        }`}
      >
        <div className="sm:hidden mt-3 mr-3 ml-4">
          <svg
            width="48"
            height="32"
            viewBox="0 0 48 32"
            ...
            class="_61de-z5tCaf"
          >
            <g filter="url(#8qgw1ewy9a)">
              <rect y="13" width="48" height="6" rx="3" fill="#A61A19"></rect>
            </g>
            <circle cx="16" cy="16" r="16" fill="#fff"></circle>
            <path
              d="M11.913 22.494c.95 0 1.722.785 1.722 1.752 0 .969-.772 1.754-1.722 1.754-.951 0-1.723-.785-1.723-1.754 0-.967.772-1.752 1.723-1.752zm8.174 0c.952 0 1.723.785 1.723 1.752 0 .969-.77 1.754-1.723 1.754-.95 0-1.72-.785-1.72-1.754 0-.967.77-1.752 1.72-1.752zm1.138-16.471s.801-.146 2.58.305L22.758 8.74c-.122-.04-.85-.135-1.411-.134-1.407 0-2.091 1.359-2.091 2.098 0 .833.859 1.345 2.057 2.03 1.72.918 2.812 2.093 2.812 3.801 0 2.983-2.257 4.632-5.108 4.846l-.085.005-9.718.022a1.248 1.248 0 0 1-.198 0l-.125.003.003-.012c-.05-.009-.098-.013-.149-.027-.602-.172-.98-.705-.842-1.204l1.276-4.616c.144-.279.476-.558 1.28-.558h6.167l-.849 2.175h-4.642l-.48 1.64 8.28-.064.082-.007c1.03-.1 1.896-.65 1.896-1.668 0-.812-.608-1.347-1.908-2.03-1.511-.833-2.939-1.944-2.939-3.823 0-2.675 2.398-5.09 5.159-5.194z"
              fill="#EE2624"
            ></path>
          </svg>
        </div>
        <a href="/" className="w-[87px] mt-1 grow sm:grow-0 h-[48px] hover:opacity-80">
          <svg viewBox="0 0 87 48" xmlns="http://www.w3.org/2000/svg" className="w-[87px] h-[48px] cursor-pointer">
            <path
        ...
        <Filter></Filter>
        <Search></Search>
        <div aria-label="tote bag" className="mt-[16px] mx-4 sm:mx-6" onClick={handleClickBag}>
          <div className="relative hover:opacity-80">
            ...
            {bag?.length !== 0 ? (
              <div className="absolute rounded-full w-[25px] border-[2px] border-red-600 text-red-500 bg-white top-[-8px] left-[14px] pl-[7px] text-[14px] font-semibold">
                {bag?.length}
              </div>
            ) : null}
          </div>
        </div>
        <div className=" hidden sm:block mt-3 mr-[2px]">
          <button
            className="h-8 min-w-[101px] pb-[3px] bg-white rounded"
            onClick={() => {
              dispatch(toggleAuthenState("login"));
            }}
          >
            <span className="text-[14px] px-3 text-slate-700 font-bold whitespace-nowrap">
